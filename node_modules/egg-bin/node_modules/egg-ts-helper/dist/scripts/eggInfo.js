...
if (utils_1.checkMaybeIsTsProj(cwd)) {
    // only require ts-node in ts project
    require('ts-node/register');
}
const framework = (utils_1.getPkgInfo(cwd).egg || {}).framework || 'egg';
...
utils_1.writeFileSync(config_1.eggInfoPath, JSON.stringify(eggInfo));
...
function getLoader(baseDir, framework) {
    ...
    const egg = utils_1.requireFile(frameworkPath) || utils_1.requireFile(framework);
    /* istanbul ignore if */
    if (!egg || !EggLoader)
        return;
...
function findEggCore(baseDir, frameworkPath) {
    ...
    if (!eggCore) {
        ...
        return utils_1.requireFile('egg-core');
    }
    return eggCore;
}
